...
from Control_surfaces import yaw_vertical_fin
...
def lambda_i_tail_forward(mu, r, R, sigh,alpha_TPP,Omega,rho,Vinf,Thrust):
    ...
    V_climb_tail=0
    lambda_G = LambdaG(Vinf,V_climb_tail,lambdaig,Omega,R,alpha_TPP)
    ...
    for r in r_all:
        for sigh_val in sigh_all:
            ...
            if alpha > tail_rotor_aero["alpha_stall"]:
                print(f"Stall detected for tail rotor at r = {r:.3f} m, alpha = {alpha:.3f} deg")
...
def Simulate_tail_rotor(tail_rotor, engine, flight_condition, coll_init, Q_rotor=78732, tol2=1e-3, delE=10):
    ...
    yaw,roll_vfin=yaw_vertical_fin(delE,rho,V_inf)
    Thrust-=yaw/vertical_stabilizers["x_arm"]

    T_init=res["T"]
    print("Thrust needed by tail:",Thrust,"T_init:", T_init)
